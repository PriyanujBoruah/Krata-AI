        ...
        try {
            ...
            processedHtml = processedHtml.replace(match[0], `
                <div class="chart-wrapper">
                    ...
                    <button class="pin-btn" onclick="shareChartToWorkspace('${chartId}')">
                        <i data-lucide="pin"></i>
                    </button>
                </div>
            `);
...
export async function processMermaidCharts(messageElement) {
    ...
    for (let i = 0; i < mermaidBlocks.length; i++) {
        ...
        try {
            ...
            // 🚀 CLEANUP: Remove the "ghost" error element Mermaid injected globally
            const ghostError = document.getElementById('d' + chartId + '-svg');
