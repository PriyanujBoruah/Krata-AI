    ...
    while (currentAttempt <= maxRetries) {
        const res = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages:[{ 
                    role: "system", 
                    content: `You are an expert DuckDB SQL Engineer. Target Table: "${activeTable}". Schema: [${schema}].
                    
                    RULES:
                    1. ALWAYS use "ILIKE" for string comparisons.
                    2. Always quote table and column names with double quotes.
                    3. FALLBACK: If the question is vague (e.g. "show data"), output: SELECT * FROM "${activeTable}" LIMIT 10
                    4. Output ONLY JSON: {"reasoning": "...", "sql": "query without semicolon"}
        ...
        try {
            ...
            return { success: true, sql: plan.sql, reasoning: plan.reasoning, data: dbResult };
        } catch (error) {
            currentAttempt++;
            sqlErrorHistory = error.message;
            if (currentAttempt > maxRetries) return { success: false, error: error.message, sql: plan.sql };
        }
    }
}
...
export async function* streamAdvancedNarrative(activeTable) {
    ...
    const techBriefResponse = await fetch(API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${MISTRAL_API_KEY}` },
        body: JSON.stringify({
